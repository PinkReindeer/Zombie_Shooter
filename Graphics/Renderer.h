...
public:
	...
	void RenderMob(int xp, int yp, float rotation, float multiplySize, EntityType type, bool shouldDraw, bool isMutation);
	void RenderBullet(int xp, int yp) const;
	void RenderParticle(int xp, int yp, float radius, Color inner, Color outer) const;
	void RenderHealthBar(int xp, int yp, int fillAmount) const;
	void RenderSoulOrb(int xp, int yp, float radius, Color inner, Color outer) const;
	...
	int GetTileMapRowSize() const { return m_Tilemap.TILE_SIZE * m_Tilemap.GetMapRows() / 2.0f * m_Tilemap.TILE_SCALE; };
	int GetTileMapColSize() const { return m_Tilemap.TILE_SIZE * m_Tilemap.GetMapCols() / 2.0f * m_Tilemap.TILE_SCALE; };

	const Texture2D& GetTexture(EntityType type) const;

	void RenderText(const char* text, int x, int y, int size, int style, int color);
